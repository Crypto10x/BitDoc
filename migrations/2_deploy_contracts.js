...
module.exports = async function(deployer) {
  // 1 Deploy DCA Token
  await deployer.deploy(DCAToken);
  const dcaToken = await DCAToken.deployed();
  // 2. Deploy DCA swap
  ...
  const usd = await MockUSD.deployed();
  ...
  await deployer.deploy(BTCExchange, btc.address,usd.address);
